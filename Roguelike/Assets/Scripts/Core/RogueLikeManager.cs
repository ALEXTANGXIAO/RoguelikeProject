...
namespace TGame
{
    public class RogueLikeManager : MonoSingleton<RogueLikeManager>
    {
        ...
        private void SetMapRandom()
        {
            ...
            for (int x = 0; x < columns; x++)
            {
                for (int y = 0; y < rows; y ++)
                {
                    ...
                    GameObject instance = Instantiate(toInstance, V3toWorld(new Vector3(x, y, 0f)), Quaternion.identity) as GameObject;
        ...
        private Vector3 RandomPosition()
        {
            ...
            return V3toWorld(randomPosition);
        ...
        public Vector3 V3toWorld(Vector3 pos)
        {
            //float x = pos.x - (float)columns / 2;
            //float y = pos.y - (float)rows / 2;
            Vector3 _pos = new Vector3(pos.x - (float)columns / 2, pos.y - (float)rows /2,0f); 
            return _pos;
        }

        public Vector2 V2toWorld(Vector2 pos)
        {
            Vector2 _pos = new Vector3(pos.x - (float)columns / 2, pos.y - (float)rows / 2);
            return _pos;
        }
    }
}
